...
- `public-dataset/` — practice-data folders for AI tool education (Claude Code, Codex, etc.), grouped under one directory: `pdf/`, `csv/`, `excel/`, `ppt/`, `image/`, `movie/`, `json/`, `audio/`, `txt/`, `zip/`, `sqlite/`, `html/`. Not wired into any app. Each subfolder has generated sample files plus a `README.md` describing what it's for and example exercises; `movie/` holds a link list (`sample_video_links.md`) to public sample mp4s instead of large video files. `ppt/` is the one folder sourced from a third party rather than generated — `creative_brief_deck.pptx` is a downloaded Canva free template, so keep it practice-only (Canva's license restricts redistributing the template file itself), and re-encode embedded images (1920px / q80) before committing any further deck: the raw download was 39MB and git history is permanent. The exercise that folder exists for is **design reproduction** (hand Claude an existing deck, get a new one in the same design), not summarizing — so its README documents the four traps that break naive reproduction (Office-default `theme1.xml` that does not match the real palette, a non-standard 20×11.25in canvas, embedded commercial fonts, and photos stored as `blipFill` inside groups rather than `<p:pic>`, which makes python-pptx's PICTURE scan return zero). `ppt/DESIGN.md` is the measured design system for that deck plus the reusable procedure/script/prompt for producing the same document from any other pptx — it is the exercise's answer key, so if the deck is ever swapped, re-run the script in it rather than hand-editing the numbers. One deliberate exception to the by-file-type grouping: `business-docs/` is a **lecture practice set** that mirrors the student's working-folder layout (`references/`, `drafts/`, `data/`) instead of splitting by type — the B2B lecture prompts point at paths like `@references/지난제안서.md`, so the folder structure itself is the payload. Keep those files where they are and keep their section headings explicit (the structure-extraction exercises depend on them); everything in there is mock data (fictional company 주식회사 세이프칩, fictional people/customers/amounts/dates).
